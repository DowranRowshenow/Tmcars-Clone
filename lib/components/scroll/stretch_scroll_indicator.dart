...
class _GlowController extends ChangeNotifier {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double height = math.min(
      size.height,
      size.width * _widthToHeightFactor,
    );
    ...
    final Paint paint = Paint()
      ..color = color.withValues(alpha: _glowOpacity.value);
    canvas.save();
...
class _StretchingOverscrollIndicatorState
    extends State<StretchingOverscrollIndicator>
    with TickerProviderStateMixin {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (notification is OverscrollNotification) {
      ...
      if (_accepted) {
        ...
        if (notification.velocity != 0.0) {
          assert(notification.dragDetails == null);
          _stretchController.absorbImpact(
            notification.velocity.abs(),
            _totalOverscroll,
          );
        } else {
          assert(notification.overscroll != 0.0);
          if (notification.dragDetails != null) {
            ...
            final double distanceForPull =
                _totalOverscroll.abs() / viewportDimension;
...
class _StretchController extends ChangeNotifier {
  ...
  static const double _stretchIntensity = 0.032;
  ...
  void absorbImpact(double velocity, double totalOverscroll) {
    ...
    _stretchSizeTween.end = math.min(
      _stretchIntensity + (_flingFriction / velocity),
      1.0,
    );
    _stretchController.duration = Duration(
      milliseconds: math
          .max(velocity * 0.02, _kMinStretchDuration.inMilliseconds)
          .round(),
    );
  ...
  void pull(double normalizedOverscroll, double totalOverscroll) {
    ...
    final _StretchDirection newStretchDirection = totalOverscroll > 0
        ? _StretchDirection.trailing
        : _StretchDirection.leading;
    if (_stretchDirection != newStretchDirection &&
